...
# =============================================================================
# PlanRun · maintain skill — 开发环境诊断与安全清理
# 用法：
#   packages/skill-provider/skills/maintain/scripts/dev-maintain.sh --diagnose
#   packages/skill-provider/skills/maintain/scripts/dev-maintain.sh --clean [--dry-run] [--interactive]
#   packages/skill-provider/skills/maintain/scripts/dev-maintain.sh --snapshot | --diff
# =============================================================================
...
# Project root: git toplevel when available, else cwd
if git rev-parse --show-toplevel >/dev/null 2>&1; then
    PROJECT_ROOT="$(git rev-parse --show-toplevel)"
else
    PROJECT_ROOT="$(pwd)"
fi
DISK_COLLECT="${SKILLS_ROOT}/disk/scripts/collect-disk.py"
DISK_SNAPSHOT_DIR="${PROJECT_ROOT}/.dsh/growth/disk-snapshots"
CONFIG_DEFAULT="${SKILL_ROOT}/config/default-protected.json"
CONFIG_OVERRIDE="${PROJECT_ROOT}/.dsh/growth/maintain-config.json"
INSTALLER_DIRS_JSON='[]'
PROTECTED_DIRS=()
CACHE_DIRS=()
EXTRA_CACHE_DIRS=()
...
diagnose_system() {
    ...
    echo ""
    echo "🔒 受保护目录见 config/default-protected.json 与 .dsh/growth/maintain-config.json"
    echo ""
...
clean_system() {
    ...
    echo "2. 受保护目录见 .dsh/growth/maintain-config.json（交互模式可确认清理安装包）"
