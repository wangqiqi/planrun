...
name: mcp
description: >-
  MCP server design — tool boundaries, naming, pagination, errors, transport;
  five-phase build flow. See reference/ guides.
...
- **只读默认安全**；写/删须明确确认边界
- **无密钥进 repo**；env 或 DSH MCP / profile 配置注入；日志不泄露 token/PII
...
- [ ] 无重复巨型「万能」工具
- [ ] 错误信息可行动
- [ ] `npx @modelcontextprotocol/inspector` 或 DSH：`GetDynamicTools` → 只读 `CallDynamicTool` 冒烟
- [ ] 写路径有确认/幂等说明
...
1. Inspector / `GetDynamicTools` 清点工具
2. 只读探查目标数据形态
...
- 在 profile MCP 配置注册 server（用户/团队批准后；勿擅自装 shadow MCP）
- **调用顺序（Agent）**：先 **`GetDynamicTools`**（按 `namespace` / `toolName` / `pattern`）取 schema → 再 **`CallDynamicTool`**；未读 schema 勿盲调
- 鉴权失败 / `needsAuth` → 对该 namespace 经 `CallDynamicTool` 完成 auth 后再试；勿对未授权 namespace 反复 auth
...
或在 DSH 会话中：`GetDynamicTools` → 对目标 tool 做一次只读 `CallDynamicTool`。
